...
else. To see how, we can make a contingency table of species by location
...
Another way is to make a graph. This is like a grouped bar chart, but slightly different because our dataframe `tbl.3` already has frequencies in it, whereas `geom_bar` expects to be counting them up. As a consequence, we instead use `geom_col`, and feed that a `y` that contains the frequencies:^[In this kind of plot, the explanatory thing goes in `x` and the thing that looks like an outcome goes in `fill`. Then you can say "out of the explanatory things, this fraction of them ended up in the response thing".]
