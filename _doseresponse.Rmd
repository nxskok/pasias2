...
This ought to be based on `plot_predictions`, but that doesn't work here because of the `response` that's not part of the dataframe. So we will be making this ourselves. Let's start with a plot of the predictions, using the prediction we did just now:
...
This is not a smooth curve like the ones `plot_predictions` makes, but that's all right, because we want to compare the predictions with the data.
