...
Extra: you could also make a plot of these. The normal thing is to use `plot_predictions`, but this time, the response variable is that matrix thing outside the dataframe, which confuses the issue. So let's make a more detailed set of predictions and plot them ourselves (effectively, doing the same thing `plot_predictions` does but building it ourselves):
